...
CLASS zcl_email_bcs_demo DEFINITION
FINAL
CREATE PUBLIC.
...
CLASS zcl_email_bcs_demo IMPLEMENTATION.
  ...
  METHOD htmleml.
    ...
    TRY.
        ...
        email->send( ).
        BREAK gaikvi02.
        DATA(lv_oid) =  email->oid( ).
        data(call_tcode) = zcl_call_tcode=>create( ).
        call_tcode->email_disp( CONV #( lv_oid ) ).

      CATCH zcx_email INTO DATA(ex).
  ...
  METHOD htmleml_bcs.
    ...
    TRY.
        ...
        "Create send request
        DATA(email) = cl_bcs=>create_persistent( ).

        LOOP AT filters-s_email INTO DATA(ls_email).
          email->add_recipient( EXPORTING
        ...
        "Set sender
        email->set_sender( cl_cam_address_bcs=>create_internet_address( 'do@not.reply' ) ).

        "Email BODY
        "Add document to send request
        email->set_document( cl_document_bcs=>create_document(
                        i_type    = 'RAW'
                        i_text    = VALUE #( ( |Hello world! My ABAP email using Class CL_BCS! { sy-uzeit TIME = USER }| ) )
                        i_subject = 'Subject' ) ).

        "Send email
        DATA(sent_to_all) = email->send( i_with_error_screen = 'X' ).
...
START-OF-SELECTION.
  ...
  DATA(email_demo) = NEW zcl_email_bcs_demo( ).
  email_demo->process( ).
